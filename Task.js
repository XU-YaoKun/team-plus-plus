...
taskRef.once("value",snapshot => { 
	...
	snapshot.forEach((child) => {
		...
		node.innerHTML=`<div class="${list}"><li id="${oneRef.key}"><div class="title">Title: ${title}</div><div class="handler">Handler: ${handler}</div></li><div class="arrows"><div class="left">⇦</div><div class="right">⇨</div></div></div>`;
		//left arrow
		node.childNodes[0].childNodes[1].childNodes[0].addEventListener('click', function(){
		...
		//right arrow
		node.childNodes[0].childNodes[1].childNodes[1].addEventListener('click', function(){
			let li = this.parentNode.parentNode;
			let status=li.className;
			if(status==="todo")
			{
		...
		//Show Edit Dialog
			node.childNodes[0].childNodes[0].addEventListener('click', function() {
				currentTask = this;
				document.getElementById("title1").value=this.childNodes[0].innerText.replace('Title: ','');
				document.getElementById("handler1").value=this.childNodes[1].innerText.replace('Handler: ','');
